...
def final_eval_v2(self: BaseTrainer):
    """
    originated from ultralytics/ultralytics/engine/trainer.py
    """
...
def strip_optimizer_v2(f: Union[str, Path] = 'best.pt', s: str = '') -> None:
    """
    Disabled half precision saving. originated from ultralytics/ultralytics/utils/torch_utils.py
    """
...
def prune(args):
    ...
    # use args.data dataset for args.epochs fine-tuning each pruning iteration step
    ...
    # Save the final pruned and fine-tuned model as .pt file
    final_model_path = f"{args.model.rsplit('.', 1)[0]}_pruned.pt"
    torch.save(model.state_dict(), final_model_path)
    print(f"Final pruned model saved as {final_model_path}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--model', default='path/to/pretrained_model_file.pt', help='Pretrained pruning target model file, e.g. /ultralytics/runs/detect/train/weights/best.pt')
    parser.add_argument('--cfg', default='default.yaml',
                        help='Pruning config file.'
                             ' This file should have same format with ultralytics/ultralytics/cfg/default.yaml')
    ...
    parser.add_argument('--data', type=str, default='path/to/custom_dataset_cfg.yaml',help='Path to the dataset configuration file')
    parser.add_argument('--epochs', type=int, default=30, help='Number of epochs for fine-tuning each pruning iteration step')
